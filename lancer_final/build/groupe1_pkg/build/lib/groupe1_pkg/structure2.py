...
class Information(Node):
    ...
    def appel_position(self, msg):
        pos_rob[0] = msg.pose.position.x
        pos_rob[1] = msg.pose.position.y
        pos_rob[2] = msg.pose.position.z
        pos_rob[3] = msg.pose.orientation.w
        pos_rob[4] = msg.pose.orientation.x
        pos_rob[5] = msg.pose.orientation.y
        pos_rob[6] = msg.pose.orientation.z

    def appel_angle(self, msg):
        angle_rob[0] = msg.position[0]
        angle_rob[1] = msg.position[1]
        angle_rob[2] = msg.position[2]
        angle_rob[3] = msg.position[3]
        angle_rob[4] = msg.position[4]
...
def trajectoire(A, B, hauteur, fonction='ligne', n=100):
    ...
    for i in range(len(P) - 1):
        traj += [(P[i+1][0]*k/n + P[i][0]*(1-k/n), P[i+1][1]*k/n + P[i][1]*(1-k/n),
                  P[i+1][2]*k/n + P[i][2]*(1-k/n) + Courbe[fonction](k/n)) for k in range(n+1)]
...
def securite(mouvement, information):
    ...
    list_point = trajectoire(pos_rob[:4], (0.2, 0., 0.), 0.05, "cadre", 40)
    tps = 0.5
    while list_point:
...
def manoeuvre(mouvement, information, n):
    ...
    list_point = trajectoire(pos_act[:4], (0., 0.18, 0.005), 0.05, "cadre", 5)
    tps = 0.5
    while list_point:
        rclpy.spin_once(information)
        if time.time() - prec_tps > tps:
    ...
    list_point = trajectoire(pos_act[:4], (0., 0.13, 0.005), 0.05, "ligne", 5)
    tps = 0.5
    while list_point:
        rclpy.spin_once(information)
        if time.time() - prec_tps > tps:
    ...
    list_point = trajectoire(pos_act[:4], (0., 0.07, 0.), 0.05, "cadre", 5)
    tps = 0.5
    while list_point:
        rclpy.spin_once(information)
        if time.time() - prec_tps > tps:
    ...
    list_point = trajectoire(pos_act[:4], (0., 0.09, 0.02), 0.05, "ligne", 5)
    tps = 0.5
    while list_point:
        rclpy.spin_once(information)
        if time.time() - prec_tps > tps:
...
def bougeage(mouvement, information, obj):
    ...
    if obj[1] >= 0:
        list_point = trajectoire(pos_act[:4], ((r - 0.05)*np.sin(
            th1 + 3*np.pi/180), (r - 0.05)*np.cos(th1 + 3*np.pi/180), 0.002), 0.05, "cadre", 5)
    else:
        list_point = trajectoire(pos_act[:4], ((r - 0.05)*np.sin(
            th1 - 3*np.pi/180), (r - 0.05)*np.cos(th1 - 3*np.pi/180), 0.002), 0.05, "cadre", 5)
    tps = 0.5
    ...
    list_point = trajectoire(pos_act[:4], (0.2, 0., 0.), 0.05, "cadre", 5)
    tps = 0.5
    while list_point:
        rclpy.spin_once(information)
        if time.time() - prec_tps > tps:
...
def main(args=None, hmax=5):
    global Pla, pos_act, angle_rob, prec_tps, pos_rob
    ...
    Pla = []
    angle_rob = [0 for i in range(7)]
    list_point = []
    prec_tps = time.time()

    pos_rob = [0., 0.2, 0.2, 0, 0, 0, 0]
        ...
        if not list_point:
            if vide:
                ...
                if obj:
                    ...
                    if not prenable:
                        bougeage(mouvement, information, obj)
                        continue
                    hauteur = 0.05
                    list_point = trajectoire(
                        pos_act, obj, hauteur, "cadre", 5)
                    tps = 0.5
                    vide = False
                else:
                ...
                list_point = trajectoire(pos_act, obj, hauteur, "cadre", 5)
                tps = 0.5
                vide = True
                n += 1
