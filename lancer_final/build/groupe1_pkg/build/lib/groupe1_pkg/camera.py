...
def cleanContours(edges_img, edges_img_canvas, contours, min_area, max_area):
    ...
    for contour in contours:
        ...
        if area > max_area or min_area > area:
            ...
            cv2.drawContours(edges_img, contour, -1,
                             color=(0, 0, 0), thickness=5)
            cv2.drawContours(edges_img_canvas, contour, -1,
                             color=(128, 128, 128), thickness=5)

        else:
            # Each contour is a huge array of points. Fortunately, cv2 can approximate its shape :
            perimeter = cv2.arcLength(contour, closed=True)
            epsilon = 0.03*perimeter
            approximate_shape_corners = cv2.approxPolyDP(
...
def findCentersGravity(img, edges_img, edges_img_canvas, min_area, max_area, robot_line, robot_column, min_cos_alpha, draw_contours=True):
    ...
    contours, hierarchy = cv2.findContours(
        edges_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    noisy_contour_present = cleanContours(
        edges_img, edges_img_canvas, contours, min_area, max_area)
    ...
    for contour in contours:
        ...
        if draw_contours:
            ...
            misaligned = isPlankMisaligned(
                min_cos_alpha, robot_line, robot_column, points_list)
            ...
            cv2.circle(img, (gravity_column, gravity_line),
                       5, chosen_color, thickness=5)
            cv2.line(img, (robot_column, robot_line),
                     (gravity_column, gravity_line), (255, 0, 0), thickness=3)
...
def convertPlankCoordinates(planks_list, nb_lines_frame, nb_columns_frame, robot_line, robot_column, distance_ratio_x, distance_ratio_y, min_cos_alpha):
    ...
    for plank in planks_list:
        ...
        plank_converted.append(isPlankMisaligned(
            min_cos_alpha, robot_line, robot_column, plank))
...
# despite its looks, this is useful for trackbars used in calibration below.
def do_nothing(useless_argument):
    pass
...
def cameraVision_calibrate(camera_number_on_pc, mtx, distorsion_coefficients):
    ...
    while(True):
        ...
        if ret:
            ...
            color_divider = 2**cv2.getTrackbarPos(
                "color divider", "parameters")
            ...
            if first_picture:
                ...
                cv2.createTrackbar("rob line", "elements", int(
                    (nb_lines-1)/20), nb_lines-1, do_nothing)
                cv2.createTrackbar("rob column", "elements", int(
                    (nb_columns-1)/2), nb_columns-1, do_nothing)
                cv2.createTrackbar("A line", "elements", int(
                    (nb_lines-1)/20), nb_lines-1, do_nothing)
                cv2.createTrackbar("A column", "elements", int(
                    (nb_columns-1)/20), nb_columns-1, do_nothing)
                cv2.createTrackbar("B line", "elements", int(
                    19*(nb_lines-1)/20), nb_lines-1, do_nothing)
                cv2.createTrackbar("B column", "elements", int(
                    19*(nb_columns-1)/20), nb_columns-1, do_nothing)
            ...
            canny, img2 = cannyEdgeDetector(
                frame, threshold1, threshold2, color_divider, thickness)
            canny_canvas = deepcopy(canny)
            plank_list = findCentersGravity(
                img2, canny, canny_canvas, min_area, max_area, robot_line, robot_column, min_cos_alpha)
            cv2.circle(img2, (robot_column, robot_line),
                       10, (255, 0, 0), thickness=4)
            cv2.circle(img2, (a_column, a_line), 10,
                       (255, 255, 0), thickness=4)
            cv2.circle(img2, (b_column, b_line), 10,
...
def cameraVision(camera_number_on_pc, mtx, distorsion_coefficients, min_area, max_area, threshold1, threshold2, crop, thickness, color_divider, robot_line, robot_column, distance_ratio_x, distance_ratio_y):
    ...
    for i in range(20):
        ...
        if ret:
            ...
            canny, img2 = cannyEdgeDetector(
                frame, threshold1, threshold2, color_divider, thickness)
            canny_canvas = deepcopy(canny)
            planks_list_1frame = findCentersGravity(
                img2, canny, canny_canvas, min_area, max_area, robot_line, robot_column, min_cos_alpha)
    ...
    planks_converted = convertPlankCoordinates(
        planks_list_final, nb_lines, nb_columns, robot_line, robot_column, distance_ratio_x, distance_ratio_y, min_cos_alpha)
...
def test():
    min_area, max_area, threshold1, threshold2, crop, thickness, color_divider, robot_line, robot_column, distance_ratio_x, distance_ratio_y = cameraVision_calibrate(
        0, mtx, distorsion_coefficients)
    planks_list = cameraVision(0, mtx, distorsion_coefficients, min_area, max_area, threshold1, threshold2,
...
class Campub(Node):
    ...
    def timer_callback(self):
        ...
        """for pl in Planches:
            plank = planche()
            plank.centre.x, plank.centre.y = pl[0][0], pl[0][1]
            plank.hd.x, plank.hd.y = pl[1][0], pl[1][1]
            plank.hg.x, plank.hg.y = pl[2][0], pl[2][1]
            plank.bd.x, plank.bd.y = pl[3][0], pl[3][1]
            plank.bg.x, plank.bg.y = pl[4][0], pl[4][1]
            plank.prenable = pl[5]
            msg.append(plank)"""
        for pl in msg.poses:
