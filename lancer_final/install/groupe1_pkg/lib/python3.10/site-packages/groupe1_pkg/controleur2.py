...
def trajectoire(A, B, hauteur, fonction='ligne', n=1):
    ...
    for i in range(len(P) - 1):
        Actions.extend([(P[i+1][0]*k/n + P[i][0]*(1-k/n), P[i+1][1]*k/n + P[i][1]*(1-k/n),
                         P[i+1][2]*k/n + P[i][2]*(1-k/n), 90) for k in range(1, n+1)])
...
def manoeuvre(n):
    ...
    Actions.append((placement[n//hmax - 1][0],
                   placement[n//hmax - 1][1], 0.02, 90))
    Actions.append((placement[n//hmax - 1][0],
                   placement[n//hmax - 1][1], 0., 90))
...
def main(args=None):
    ...
    hauteur = 0.05
    cas_boucl = True
    vitesse = 0.05  # vitesse de mouvement du bras en m/s
