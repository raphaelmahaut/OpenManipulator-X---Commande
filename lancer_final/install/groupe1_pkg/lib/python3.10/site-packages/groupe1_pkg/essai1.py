...
def mgi2(x, y, z, th):  # modèle géométrique inverse en fixant l'angle du premier bras
    ...
    zb = z - (Dim[0] + Dim[1]*np.cos(np.pi*th/(2*90)))
    r = np.sqrt(x**2 + y**2) - Dim[1]*np.sin(np.pi*th/(2*90))
    hp = np.sqrt(zb**2 + r**2)
...
def trajx(t):
    if t < 0.4*2:
        coefs = [-0.9155,  1.5869,  -0.5859,    0,     0,     0.08]
    else:
        t = t - 0.4*2
        coefs = [-0.4973,   1.5643,  -1.3672, 0, 0.25,   0.13]
    P = 0
    for i in coefs:
        P *= t
        P += i
...
def trajz(t):
    if t < 0.4*2:
        coefs = [-0.9155,  1.5869,  -0.5859,    0,     0,     0.03]
    else:
        t = t - 0.4*2
        coefs = [-0.4973,   1.5643,  -1.3672, 0, 0.25,   0.08]
    P = 0
    for i in coefs:
        P *= t
        P += i
...
def lanc(mouvement, tps):
    # convertit les coordonnées en position angulaire
    global enregistre

    
    th1, th2, th3, th4 = mgi(trajx(0), 0., trajz(0), 90)
    ...
    for t in np.linspace(0., 1.8, n):
        while time.time()-origine < t:
            continue
        if t > 0.8 and not tir:
            tir = True
            ouverture = 0.01
            prise(mouvement, ouverture, 0.001)
        if t < 0.4*2:
            th1, th2, th3, th4 = mgi(trajx(0.2 + t), 0., trajz(0.2 + t), 90 - t*60)
        else:
            th1, th2, th3, th4 = mgi(trajx(0.2 + t), 0., trajz(0.2 + t), 90 - (2.0 - t)*40)
        mouvement.depl_requete(th1, th2, th3, th4, 0.2)
        while rclpy.ok():
            rclpy.spin_until_future_complete(
                mouvement, mouvement.depl_future)
            if mouvement.depl_future.done():
